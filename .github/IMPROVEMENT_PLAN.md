...
| 25 | ✅ | — | PDF audit complete. 21 commands reviewed. Items 5 (communication method) and 6 (baud rate) are N/A — changing either via HA would break the UART connection. Item 20 (heartbeat control) deferred — requires a new RX response handler for periodic heartbeat packets; not enough user value to justify the complexity now. Three actionable gaps found: items 30, 31, 32 below. |
