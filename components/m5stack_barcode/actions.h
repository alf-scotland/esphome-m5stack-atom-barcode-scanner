...
template<typename... Ts> class SetModeAction : public Action<Ts...> {
 public:
  ...
  void set_mode(const std::string &mode_str) {
    if (mode_str == "host") this->mode_ = OperationMode::HOST;
    else if (mode_str == "level") this->mode_ = OperationMode::LEVEL;
    else if (mode_str == "pulse") this->mode_ = OperationMode::PULSE;
    else if (mode_str == "continuous") this->mode_ = OperationMode::CONTINUOUS;
    else if (mode_str == "auto_sense") this->mode_ = OperationMode::AUTO_SENSE;
    else ESP_LOGW(TAG_ACTION, "Unknown operation mode: %s", mode_str.c_str());
  }
...
template<typename... Ts> class SetTerminatorAction : public Action<Ts...> {
 public:
  ...
  void set_terminator(const std::string &term_str) {
    if (term_str == "none") this->terminator_ = Terminator::NONE;
    else if (term_str == "crlf") this->terminator_ = Terminator::CRLF;
    else if (term_str == "cr") this->terminator_ = Terminator::CR;
    else if (term_str == "tab") this->terminator_ = Terminator::TAB;
    else if (term_str == "crcr") this->terminator_ = Terminator::CRCR;
    else if (term_str == "crlfcrlf") this->terminator_ = Terminator::CRLFCRLF;
    else ESP_LOGW(TAG_ACTION, "Unknown terminator: %s", term_str.c_str());
  }
...
template<typename... Ts> class SetLightModeAction : public Action<Ts...> {
 public:
   ...
   */
  void set_mode(const std::string &mode_str) {
    if (mode_str == "on_when_reading") this->mode_ = LightMode::ON_WHEN_READING;
    else if (mode_str == "always_on") this->mode_ = LightMode::ALWAYS_ON;
    else if (mode_str == "always_off") this->mode_ = LightMode::ALWAYS_OFF;
    else ESP_LOGW(TAG_ACTION, "Unknown light mode: %s", mode_str.c_str());
  }
...
template<typename... Ts> class SetLocateLightModeAction : public Action<Ts...> {
 public:
  ...
  void set_mode(const std::string &mode_str) {
    if (mode_str == "on_when_reading") this->mode_ = LocateLightMode::ON_WHEN_READING;
    else if (mode_str == "always_on") this->mode_ = LocateLightMode::ALWAYS_ON;
    else if (mode_str == "always_off") this->mode_ = LocateLightMode::ALWAYS_OFF;
    else ESP_LOGW(TAG_ACTION, "Unknown locate light mode: %s", mode_str.c_str());
  }
...
template<typename... Ts> class SetSoundModeAction : public Action<Ts...> {
 public:
   ...
   */
  void set_mode(const std::string &mode_str) {
    if (mode_str == "disabled") this->mode_ = SoundMode::SOUND_DISABLED;
    else if (mode_str == "enabled") this->mode_ = SoundMode::ENABLED;
    else ESP_LOGW(TAG_ACTION, "Unknown sound mode: %s", mode_str.c_str());
  }
...
template<typename... Ts> class SetBuzzerVolumeAction : public Action<Ts...> {
 public:
  ...
  void set_volume(const std::string &volume_str) {
    if (volume_str == "high") this->volume_ = BuzzerVolume::VOLUME_HIGH;
    else if (volume_str == "medium") this->volume_ = BuzzerVolume::MEDIUM;
    else if (volume_str == "low") this->volume_ = BuzzerVolume::VOLUME_LOW;
    else ESP_LOGW(TAG_ACTION, "Unknown buzzer volume: %s", volume_str.c_str());
  }
