...
CONF_TEXT_SENSOR = "text_sensor"  # Define this locally instead of importing from esphome.const
...
BarcodeScanner = m5stack_barcode_ns.class_(
    "BarcodeScanner", cg.Component, uart.UARTDevice
)
...
SetTerminatorAction = m5stack_barcode_ns.class_("SetTerminatorAction", automation.Action)
SetLightModeAction = m5stack_barcode_ns.class_("SetLightModeAction", automation.Action)
SetLocateLightModeAction = m5stack_barcode_ns.class_("SetLocateLightModeAction", automation.Action)
SetSoundModeAction = m5stack_barcode_ns.class_("SetSoundModeAction", automation.Action)
SetBuzzerVolumeAction = m5stack_barcode_ns.class_("SetBuzzerVolumeAction", automation.Action)
...
async def barcode_start_to_code(config, action_id, template_arg, args):
...
async def barcode_stop_to_code(config, action_id, template_arg, args):
...
@automation.register_action(
    ...
    cv.Schema({
        ...
        cv.Required(CONF_MODE): cv.templatable(cv.enum(OPERATION_MODES, lower=True)),
    })
)
async def barcode_set_mode_to_code(config, action_id, template_arg, args):
...
@automation.register_action(
    ...
    cv.Schema({
        cv.GenerateID(): cv.use_id(BarcodeScanner),
        cv.Required(CONF_TERMINATOR): cv.templatable(cv.enum(TERMINATORS, lower=True)),
    })
)
async def barcode_set_terminator_to_code(config, action_id, template_arg, args):
...
async def barcode_set_light_mode_to_code(config, action_id, template_arg, args):
...
async def barcode_set_locate_light_mode_to_code(config, action_id, template_arg, args):
    """Register set locate light mode action."""
    parent = await cg.get_variable(config[CONF_ID])
    var = cg.new_Pvariable(action_id, template_arg, parent)
    
    template_ = await cg.templatable(config[CONF_LOCATE_LIGHT_MODE], args, cg.std_string)
    cg.add(var.set_mode(template_))
    return var
...
async def barcode_set_sound_mode_to_code(config, action_id, template_arg, args):
...
async def barcode_set_buzzer_volume_to_code(config, action_id, template_arg, args):
