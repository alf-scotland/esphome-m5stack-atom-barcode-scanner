...
std::unique_ptr<CommandBase> CommandFactory::create_version_command() {
  return make_unique<SimpleCommand>(Commands::GET_VERSION, Commands::GET_VERSION_SIZE,
                                    "Get version", nullptr, nullptr, ResponseType::VERSION);
}
...
std::unique_ptr<CommandBase> CommandFactory::create_mode_command(OperationMode mode) {
  ...
  return make_unique<Command<OperationMode>>(cmd_data, Commands::Mode::SIZE, mode,
                                             operation_mode_to_string(mode), nullptr);
}
...
std::unique_ptr<CommandBase> CommandFactory::create_terminator_command(Terminator term) {
  ...
  return make_unique<Command<Terminator>>(cmd_data, Commands::Terminator::SIZE, term,
                                          terminator_to_string(term), nullptr);
}
...
std::unique_ptr<CommandBase> CommandFactory::create_light_command(LightMode mode) {
  ...
  return make_unique<Command<LightMode>>(cmd_data, Commands::Light::SIZE, mode,
                                         light_mode_to_string(mode), nullptr);
}
...
std::unique_ptr<CommandBase> CommandFactory::create_sound_command(SoundMode mode) {
  ...
  return make_unique<Command<SoundMode>>(cmd_data, Commands::Sound::SIZE, mode,
                                         sound_mode_to_string(mode), nullptr);
}
...
std::unique_ptr<CommandBase> CommandFactory::create_volume_command(BuzzerVolume volume) {
  ...
  return make_unique<Command<BuzzerVolume>>(cmd_data, Commands::Volume::SIZE, volume,
                                            buzzer_volume_to_string(volume), nullptr);
}
