...
void BarcodeScanner::loop() {
  ...
  if (this->waiting_for_ack_) {
    ...
    if (!this->rx_buffer_.empty() &&
        this->is_ack_sequence_(this->rx_buffer_.data(), this->rx_buffer_.size())) {
  ...
  if (this->expected_response_ == ResponseType::BARCODE ||
      this->expected_response_ == ResponseType::NONE) {
...
void BarcodeScanner::dump_config() {
  ...
  ESP_LOGCONFIG(TAG_SCANNER, "  Operation Mode: %s",
                operation_mode_to_string(this->operation_mode_));
  ...
  ESP_LOGCONFIG(TAG_SCANNER, "  Locate Light Mode: %s",
                locate_light_mode_to_string(this->locate_light_mode_));
  ...
  if (this->version_sensor_ != nullptr && this->version_sensor_->has_state()) {
    ESP_LOGCONFIG(TAG_SCANNER, "  Firmware Version: %s",
                  this->version_sensor_->get_state().c_str());
  }
}

// Buffer Management Methods
void BarcodeScanner::clear_buffer_() {
  this->rx_buffer_.clear();
}

void BarcodeScanner::set_expected_response_(ResponseType type) {
  this->expected_response_ = type;
}
...
void BarcodeScanner::queue_command(std::unique_ptr<CommandBase> command) {
  ...
  if (this->command_queue_.size() >= MAX_QUEUE_SIZE) {
    ESP_LOGW(TAG_SCANNER, "Command queue full (size=%u), dropping command: %s",
             this->command_queue_.size(), command->get_description());
    return;
  }
...
void BarcodeScanner::process_barcode_() {
  ...
  if (this->operation_mode_ != OperationMode::HOST) {
    ...
    if (data_length >= 7 &&
        this->is_ack_sequence_(this->rx_buffer_.data(), data_length, data_length - 7)) {
      data_length -= 7;
    }
  }
  ...
  if (data_length > MAX_BARCODE_LENGTH) {
    ESP_LOGW(TAG_SCANNER, "Barcode too long (%u bytes), truncating to %u bytes", data_length,
             MAX_BARCODE_LENGTH);
...
void BarcodeScanner::process_version_() {
  ...
  if (version_length > MAX_VERSION_LENGTH) {
    ESP_LOGW(TAG_SCANNER, "Version string too long (%u bytes), truncating to %u bytes",
...
std::unique_ptr<CommandBase> BarcodeScanner::create_start_command() {
  return CommandFactory::create_start_command();
}

std::unique_ptr<CommandBase> BarcodeScanner::create_stop_command() {
  return CommandFactory::create_stop_command();
}
